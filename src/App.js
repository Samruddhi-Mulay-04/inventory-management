...
const ProductTableRow = (props) => {
  ...
  return (
    <tr>
      ...
      <td><img style={{ height: '8em' }} target="_blank" src={props.product.imageURL} /></td>
    </tr>
  );
}
...
class ProductsTab extends React.Component {
  ...
  handleOnChangeSearch = async (e) => {
    ...
    if (this.state.inventory) {
      let inventory = this.props.inventory.categories['all'];
      if (inventory) {
        let search = await arraySearch(inventory, value);
        this.setState(
          {
            inventory: {
              categories: {
...
const arraySearch = (array, keyword) => {
  ...
  return array.filter(value => {
    return value.category.toLowerCase().match(new RegExp(searchTerm, 'g')) ||
...
class InventoryManagementApp extends React.Component {
  ...
  componentDidMount() {
    ...
    axiosInstance.get('/get-all-products').then(res => {
      console.log(res.data);
      res.data.map(product => {
        inventory.categories['all'].push(product);
      })
    }).catch(err => {
      console.log(err);
    });
  ...
  addNewProduct(product) {
    ...
    let inventory = this.state.inventory;
    inventory.categories['all'].push(product);
    axiosInstance.post('add-product', product).then(res => {
